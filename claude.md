...
1. **서버 API 라우트 보안 적용:** 현재 `/api/data` 등의 주요 데이터 접근 API는 로그인하지 않아도 접근 가능한 상태입니다. 발급된 JWT 토큰을 프론트엔드에서 헤더에 담아 전송하고, 백엔드 미들웨어에서 이를 검증하는 로직을 추가해야 합니다.
2. **Google Calendar API 연동:** 일별 시간표 변경 사항(`이동(IN)`, `이동(OUT)`)이 발생할 때마다, Google Calendar API를 활용하여 학교 공식 캘린더나 교사 개인 캘린더에 연동하는 기능을 구현합니다.
3. **OCI 운영 서버 배포 적용:** 이번에 추가된 Google OAuth 클라이언트 ID(도메인 URI)와 `.env` 설정을 OCI 서버 환경에도 반영하여 프로덕션 서버 구글 로그인 테스트를 진행합니다.
